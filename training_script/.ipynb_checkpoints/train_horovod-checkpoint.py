...
labels_to_keep = [4, 5, 6, 7, 11, 8]
data = data[data['category'].isin(labels_to_keep)]
...
text_ids = [tokenizer.encode(text, max_length=256, pad_to_max_length=True) for text in texts]
...
train_x, test_val_x, train_y, test_val_y = train_test_split(text_ids, labels, random_state=123, test_size=0.2)
train_m, test_val_m = train_test_split(att_masks, random_state=123, test_size=0.2)

test_x, val_x, test_y, val_y = train_test_split(test_val_x, test_val_y, random_state=123, test_size=0.5)
test_m, val_m = train_test_split(test_val_m, random_state=123, test_size=0.5)

enc = LabelEncoder()
enc.fit(train_y)
train_y = enc.transform(train_y)
test_y = enc.transform(test_y)
val_y = enc.transform(val_y)
...
optimizer = AdamW(optimizer_grouped_parameters, lr=learning_rate*lr_scaler, eps=adam_epsilon)
...
for n in range(num_epochs):
    ...
    print ("\nTrain loss after itaration %i: %f" % (n, train_loss))
    avg_train_loss = metric_average(train_loss, 'avg_train_loss')
    print ("Average train loss after iteration %i: %f\\n" % (n, avg_train_loss))
    ...
    with torch.no_grad():
        ...
        print ("Average validation loss after iteration %i: %f\\n" % (n, avg_val_loss))
        val_losses.append(avg_val_loss)
    
    end_time = time.time()
    epoch_mins, epoch_secs = epoch_time(start_time, end_time)
    print(f'Epoch: {n+1:02} | Time: {epoch_mins}m {epoch_secs}s')
